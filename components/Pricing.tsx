    ...
    <section className="bg-paper px-6 py-36 sm:py-48">
      <div className="mx-auto max-w-5xl">
            ...
            <FadeIn key={plan.name} delay={i * 0.12} className="h-full">
              <article
                className={`flex h-full flex-col border-t pt-10 ${
                  plan.featured ? "border-candle" : "border-line"
                }`}
              >
                <h3 className="font-mincho text-lg font-medium tracking-[0.25em] text-ink">
                  {plan.name}
                </h3>
                <p className="mt-5 font-mincho text-xl tracking-[0.15em] text-ink">
                  {plan.price}
                </p>
                <p className="mt-6 text-sm leading-loose tracking-[0.05em] text-sumi">
                  {plan.description}
                </p>
                <ul className="mt-10 space-y-4">
                  {plan.features.map((feature) => (
                    <li
                      key={feature}
                      className="text-sm leading-relaxed tracking-[0.05em] text-sumi"
                    >
                      — {feature}
                    </li>
                  ))}
        ...
        <FadeIn className="mt-16 text-center">
          <p className="text-xs tracking-[0.2em] text-mist">{plans.note}</p>
        </FadeIn>
      </div>
    </section>
  );
